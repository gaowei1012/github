/*
 * @Author: your name
 * @Date: 2019-12-29 16:49:42
 * @LastEditTime: 2019-12-29 16:52:12
 * @LastEditors: your name
 * @Description: In User Settings Edit
 * @FilePath: /github/js/model/TrendingUtil.js
 */
...
export default class TrendingUtil {
  static htmlToRepo(responseData) {
    responseData = responseData
      .substring(
        responseData.indexOf('<li class="repo-list-item'),
        responseData.indexOf('</ol>'),
      )
      .replace(/\n/, '');
    var repos = [];
  ...
  static parseRepoBaseInfo(repo, htmlBaseInfo) {
    ...
    var description = this.parseContentOfNode(
      htmlBaseInfo,
    ...
    if (index !== -1) {
      ...
      var emoji = description.substring(
        description.indexOf('>') + 1,
        indexEmoji,
      );
      description =
        emoji + description.substring(indexEmoji + '</g-emoji>'.length);
    }
